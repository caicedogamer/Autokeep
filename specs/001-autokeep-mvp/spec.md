...
### User Story 3 - Import CSV/JSON files with flexible structure inference (Priority: P1)

The operator needs to bring in historical or externally produced records from heterogeneous sources (bank statements, ERP exports, internal spreadsheets, prior accounting tools) without pre-conforming each file to a rigid template. They select a CSV or JSON file, AutoKeep parses it, **infers** what each column represents (date, amount, type, category, description, counterparty, currency, or metadata) from header names + value patterns, shows a preview with the proposed mapping and per-column confidence, lets the operator **confirm or correct** the mapping (and disambiguate when two columns are plausible candidates for the same role), then validates every row against the normalized internal model BEFORE saving anything, surfaces a per-row report of what is valid and what is rejected with reasons, and lets the operator confirm import of the valid rows (or cancel everything). Unmapped columns are preserved as `extraMetadata` on the resulting records so no information is silently lost.

**Why this priority**: Import unblocks realistic dataset sizes and is the most error-prone flow in bookkeeping tools. Failing import quality destroys data trust on day one. A rigid schema forces operators to pre-process every file, which is the exact friction the flexible inference flow removes.

**Independent Test**: Can be fully tested by importing (a) a file whose column names and order match the canonical shape — mapping is auto-confirmed without operator intervention; (b) a heterogeneous file with non-canonical column names — preview shows inferred mapping + confidence and the operator confirms or corrects; (c) a file with ambiguous columns (e.g., two numeric columns that could both be `amount`) — preview marks the ambiguity and the operator picks; (d) a file missing a required role even after manual mapping — import is blocked with an actionable message; (e) a structurally unparseable file (corrupt bytes, encoding failure, malformed JSON) — rejected at the parser boundary with zero records touched.

**Acceptance Scenarios**:

1. **Given** a CSV or JSON file whose columns the inferrer recognizes with high confidence (≥ 0.8 for every required role and no ambiguity), **When** the operator selects it, **Then** AutoKeep shows a preview of the first N rows with the auto-detected mapping highlighted and confidence indicators per column; the operator confirms once and the import proceeds to the per-row validation report.
2. **Given** a file with non-standard column names or partial coverage (some columns recognized, others not, no ambiguity in required roles), **When** the operator selects it, **Then** the preview marks unmapped columns as `metadata` (or `ignore`), the operator may reassign any column to any semantic role via a dropdown, and the import proceeds once all required roles are satisfied.
3. **Given** a file where two or more columns are plausible candidates for the same role (difference in confidence < 0.1), **When** the preview renders, **Then** those columns are marked with an `AMBIGUOUS_ROLE` warning, no role is auto-applied for the ambiguity, and the operator MUST disambiguate before proceeding.
4. **Given** a file where a required role (`date`, `type`, `amount`, `category`, `description`) cannot be mapped to any column even after manual confirmation, **When** the operator attempts to proceed, **Then** the import is blocked with a `MISSING_REQUIRED_ROLE` message naming the missing role(s); no record is written.
5. **Given** a large import file (5,000 rows × up to 20 columns), **When** inference, preview, validation, and import run, **Then** the UI remains responsive (p95 inference + preview ≤ 1.5 s per SC-018), shows progress for each stage, and the operator can cancel at any point before commit.
6. **Given** a file that includes a currency column whose values differ from the workspace currency, **When** inference completes, **Then** the operator is warned and required to confirm; mixed currencies within the same file are rejected before commit (consistent with the single-reporting-currency assumption).
7. **Given** a structurally unparseable file (corrupt bytes, encoding failure, malformed JSON, unsupported top-level shape), **When** the operator selects it, **Then** the file is rejected at the parser boundary with an explanation and zero records are touched.
8. **Given** a file with columns that map to canonical roles plus additional unmapped columns, **When** the operator confirms the import, **Then** each created `FinancialRecord` carries the unmapped column values under `extraMetadata`, and a subsequent export (US4) round-trips those metadata fields without loss.
...
- **Locale-specific numbers and dates** in imported files (commas vs. dots as decimal separators, DD/MM/YYYY vs. MM/DD/YYYY). The inferrer detects the dominant format per column from the sample and surfaces it in the preview; rows whose value does not match the column's confirmed format are rejected at the row level with an explicit reason (not at the file level).
- **Currency**: the MVP assumes a single reporting currency (see Assumptions); imports whose currency column differs from the workspace currency are surfaced with a `CURRENCY_DIFFERS_FROM_WORKSPACE` warning and require operator confirmation; imports with mixed currencies within the same file are rejected before commit.
...
- **Missing or synthetic header**: if the file has no header row (first row is data), the inferrer synthesizes column names (`col_1`, `col_2`, …) and the operator MUST assign every required role manually before proceeding.
- **Semantically equivalent headers in different languages**: headers like `amount` / `monto` / `importe` / `total` MUST all map to the `amount` role with high confidence; the heuristics MUST be bilingual (es/en) at minimum.
- **Column with mixed value types** (text mixed into a numeric column, or mixed date formats in a date column): the inferrer reports the dominant type and the mismatch rate; the operator may proceed (atypical rows will be flagged at row validation) or remap.
- **Operator overrides a high-confidence auto-mapping**: the system accepts the manual decision without protest but records both the auto-inferred and the operator-confirmed mapping in `ImportBatch.mappingDecision` (with `source: "mixed"`) so the change is auditable.
- **Unmapped columns**: columns that the operator leaves as `metadata` are preserved on each created record as `extraMetadata` (subject to the per-record size cap); columns explicitly set to `ignore` are dropped permanently.
...
### Technical Risks — Flexible Import (US3)

- **R-IMP-1 Incorrect inference**: the system maps a column to the wrong semantic role and the operator does not notice (e.g., a "balance" column is mapped to `amount`). *Mitigation*: per-column confidence is always visible; ambiguities are explicitly flagged (FR-046); `ImportBatch.mappingDecision` is persisted for audit so a wrong mapping can be traced post-commit.
- **R-IMP-2 Persistent ambiguity**: multiple columns are equally plausible for the same required role and the heuristics cannot decide. *Mitigation*: the system refuses to auto-pick when the confidence gap is < 0.1 and forces a manual decision (FR-046).
- **R-IMP-3 Performance degradation on wide or sparse files**: files with > 50 columns or with very long string values can slow inference. *Mitigation*: inference samples the first 200 rows only and runs in the import worker; SC-018 caps p95 inference + preview at 1.5 s on the reference profile.
- **R-IMP-4 Inconsistent datasets across files**: two files for the same dataset use different category names or counterparty spellings, producing fragmented records. *Mitigation*: documented as a known MVP limitation; the operator can normalize after-the-fact through the records edit flow and the existing merge-categories path. Cross-file canonicalization is post-MVP.
- **R-IMP-5 Silent type mismatch within a column**: a column marked as numeric contains a handful of text values; the dominant-type heuristic accepts the column. *Mitigation*: row-level validation reports `INVALID_AMOUNT_FORMAT` (etc.) for the atypical rows; the mapping preview displays the mismatch rate so the operator can choose to remap rather than discard rows.
- **R-IMP-6 Extra metadata leaks PII via export**: an unmapped column containing sensitive data is preserved as `extraMetadata` and later round-trips through unencrypted export (US4). *Mitigation*: the mapping preview surfaces every column that will be preserved as metadata so the operator can explicitly mark them as `ignore`; the export dialog repeats the warning when `extraMetadata` is present in the active filter.
...
**Import (US3) — Flexible structure inference**

- **FR-011**: System MUST support importing records from CSV and JSON files of heterogeneous structure (varying column names, ordering, and presence of optional fields across files).
- **FR-012**: System MUST validate every row against the normalized internal model AFTER operator-confirmed column mapping and BEFORE writing any record to storage. Structural validation at the file level (parseability, encoding) MUST also run first; the parser MUST NOT make assumptions about column meaning.
- **FR-013**: System MUST present a per-row validation report distinguishing valid rows from invalid rows, and MUST state the rejection reason for each invalid row. The report MUST include the operator-confirmed mapping that produced the validation result so the report is interpretable in isolation.
...
- **FR-015**: System MUST reject files that fail **parser-level** validation (unparseable bytes, encoding failure, malformed JSON, unsupported top-level shape) without writing any records. Header shape mismatch is **no longer** a structural-rejection reason — files with unfamiliar column names are surfaced in the mapping confirmation step instead.
- **FR-016**: System MUST run parsing, inference, mapping confirmation, validation, and ingestion without freezing the UI; progress MUST be visible at every stage and a cancel action MUST be available before commit.

**Import flexibility (US3 extended)**

- **FR-042 — Column inference**: System MUST infer, for every column in an imported file, (a) a data type (string / number / date / enum / boolean) by sampling values, and (b) a semantic role (`date`, `amount`, `type`, `category`, `description`, `counterparty`, `currency`, or `metadata`) by combining header-name heuristics with value-pattern matching. The system MUST report a confidence score in `[0, 1]` and an ordered list of alternative role candidates per column.
- **FR-043 — Mapping confirmation**: System MUST present the inferred mapping to the operator before any validation or write, and MUST allow the operator to reassign any column to any semantic role (including `ignore` to drop the column and `metadata` to preserve it as `extraMetadata`). The system MUST record whether the final mapping was produced automatically, manually, or as a mix, with a confirmation timestamp.
- **FR-044 — Metadata preservation**: Columns not mapped to a canonical role MUST be preserved on each resulting `FinancialRecord` as `extraMetadata` (key-value strings), subject to the per-record size limits documented in `data-model.md`. Metadata fields MUST round-trip through CSV/JSON export (US4).
- **FR-045 — Partial-structure tolerance**: Files missing optional roles (`counterparty`, `currency`) MUST be accepted; files missing a required role (`date`, `type`, `amount`, `category`, `description`) even after manual mapping MUST be rejected with an actionable `MISSING_REQUIRED_ROLE` message naming the missing role(s). The system MUST NOT fabricate defaults for required roles.
- **FR-046 — Ambiguity detection**: When two or more columns are plausible candidates for the same required role (the second-best candidate has confidence within 0.1 of the best), the system MUST flag the ambiguity explicitly with an `AMBIGUOUS_ROLE` warning and MUST NOT auto-assign the role until the operator picks. Below-threshold confidence (< 0.5 for a required role) MUST be flagged as `LOW_CONFIDENCE`.
- **FR-047 — Decoupled normalization**: The transformation from the external file shape to the internal `FinancialRecord` model MUST live in a normalization layer that is separate from the parser, the inferrer, and the validator. Each stage MUST be independently testable.
- **FR-048 — Engine extensibility**: The mapping strategy MUST be exposed through a `ColumnMapper` interface so that replacing the heuristic implementation (e.g., with a future AI-based strategy) does NOT require changes to the parser, the normalizer, the validator, or the import service. The MVP ships with one implementation (`HeuristicColumnMapper`); additional implementations MUST be drop-in.
- **FR-049 — Currency inference with confirmation**: If the imported file contains a column identifiable as currency, the system MUST verify that all values equal the workspace currency. If any value differs, the system MUST surface a `CURRENCY_DIFFERS_FROM_WORKSPACE` warning and require the operator to confirm before commit. A file that contains mixed currencies within its rows MUST be rejected before commit (consistent with the single-reporting-currency assumption).
...
- **Import Batch**: a logical record of one import operation. Attributes: source filename, format (CSV | JSON), timestamp, total/valid/invalid row counts, per-row error report, operator confirmation outcome (imported valid | cancelled | rejected-structural), the **inference report** (per-column type/role/confidence/alternatives) produced by the `ColumnMapper`, and the **mapping decision** (the confirmed column → role mapping, its source `auto | manual | mixed`, the warnings raised during inference, and the confirmation timestamp). Used for traceability of imported data and for auditing the mapping after the fact.
- **Validation Report**: produced for every import. Attributes: per-row status, rejection reasons (referencing the operator-confirmed mapping), parser-level rejection summary if any, and the row-level format-mismatch rate per column. Not persisted as a long-lived entity in the MVP but MUST be visible to the operator before commit.
- **Column Inference**: per-column output of the `ColumnMapper` stage. Attributes: column index, original header (or synthesized name), inferred data type (string | number | date | enum | boolean), inferred semantic role (`date` | `amount` | `type` | `category` | `description` | `counterparty` | `currency` | `metadata`), confidence in `[0, 1]`, and an ordered list of alternative role candidates with their own confidence scores. Drives the mapping preview UI.
- **Mapping Decision**: the operator-confirmed mapping that the normalizer and validator consume. Attributes: column-index → semantic role (including `ignore` and `metadata`), source flag (`auto | manual | mixed`), list of warnings (`AMBIGUOUS_ROLE`, `LOW_CONFIDENCE`, `MIXED_TYPE_COLUMN`, `MISSING_REQUIRED_ROLE`, `CURRENCY_DIFFERS_FROM_WORKSPACE`), and the confirmation timestamp. Persisted on `ImportBatch` for audit.
...
- **SC-017**: Across a reference set of **20 heterogeneous import files** (bank statements, ERP exports, manual spreadsheets in es and en, varied column orders), the `HeuristicColumnMapper` produces a correct mapping for the five required roles **without manual intervention in ≥ 80%** of columns. Files where confidence is below threshold or ambiguity is flagged are excluded from the auto-success count but MUST resolve correctly after a single manual confirmation step.
- **SC-018**: For an imported file of **5,000 rows × up to 20 columns**, the inference + preview stage completes in **p95 ≤ 1.5 s** on the reference profile (4-core CPU at ~2.4 GHz, 8 GB RAM) and the UI remains interactive throughout (operator can cancel at any time).
- **SC-019**: Columns preserved as `extraMetadata` at import time round-trip through a subsequent CSV/JSON export and re-import in **100% of test scenarios** without loss, subject to the per-record metadata size cap documented in `data-model.md`.
- **SC-020**: The only motive for **file-level** rejection is a parser-level failure (unparseable bytes, encoding failure, malformed JSON, unsupported top-level shape) in **100% of test scenarios**. Every other inconsistency MUST be resolved at the row level with an explicit reason or surfaced as a mapping warning that the operator can resolve before commit.
...
- **Import schema is inferred with operator confirmation.** Files conforming to heterogeneous structures are supported via a column-inference + mapping-confirmation step before any persistence. There is no required canonical header; the only file-level rejection is parser-level failure (unparseable bytes, malformed JSON). Operator confirmation of the proposed mapping is mandatory before validation runs. A future AI-based mapping strategy may replace the heuristic implementation through the `ColumnMapper` interface without affecting parsing, normalization, or validation.
- **Date and decimal formats in imports are inferred per column and confirmed.** The inferrer detects the dominant date/decimal format per column from sampled rows and surfaces it in the preview; rows that do not match the confirmed format are rejected at the row level with an explicit reason rather than guessed.
